...
def create_track(
        user, license_, genre, album, cover, file, title='t1',
):
    track = models.Track.objects.create(
        user=user, license=license_, album=album,
        cover=cover, file=file, title=title)
...
def create_comment(user, track, text='t1'):
    comment = models.Comment.objects.create(
        user=user, track=track, text=text)
    comment.save()
    return comment
...
def create_playlist(user, tracks, cover, title='t1'):
    playlist = models.Playlist.objects.create(
        user=user, cover=cover, title=title)
...
class AudioLibraryTests(TestCase):
    ...
    def setUp(self):
        ...
        self.track = create_track(
            user=self.user1, license_=self.license, genre=self.genre,
    ...
    def test_create_album(self):
        ...
        album = models.Album.objects.create(
            user=self.user1, name='test_name2', description='d2',
            cover=image,
        )
    ...
    def test_create_track(self):
        ...
        track = create_track(
            user=self.user1, license_=self.license, genre=self.genre,
            album=self.album, cover=image, file=file)
    ...
    def test_create_playlist(self):
        ...
        playlist = create_playlist(
            self.user2, self.track, cover=image, title='t3')
    ...
    def test_str_track(self):
        self.assertEqual(self.track.__str__(), f"{self.track.user} - {self.track.title}")

    def test_str_comment(self):
        self.assertEqual(self.comment.__str__(), f"{self.comment.user} - {self.comment.track.title}")

    def test_str_playlist(self):
        self.assertEqual(self.playlist.__str__(), f"{self.playlist.user} - {self.playlist.title}")
